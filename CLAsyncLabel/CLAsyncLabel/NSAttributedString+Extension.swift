...
//  CKD
//
//  Created by Chen JmoVxia on 2022/4/8.
...
extension NSAttributedString {
    ...
    ///   - text: 文字
    ///   - attributes: 属性
    convenience init(text: String, attributes: ((_ item: AttributesItem) -> Void)? = nil) {
    ...
    ///   - image: 图片
    ///   - bounds: 位置大小
    convenience init(image: UIImage, bounds: CGRect, attributes: ((_ item: AttributesItem) -> Void)? = nil) {
        let item = AttributesItem()
        attributes?(item)
        let attch = NSTextAttachment()
        attch.image = image
        attch.bounds = bounds
        let attributedString = NSMutableAttributedString(attributedString: .init(attachment: attch))
        attributedString.addAttributes(item.attributes, range: NSRange(location: 0, length: attributedString.length))
...
extension NSAttributedString {
    class AttributesItem {
        ...
        /// 背景颜色
        @discardableResult
        func backgroundColor(_ value: UIColor) -> AttributesItem {
        ...
        /// 段落缩进，不包含首行
        @discardableResult
        func headIndent(_ value: CGFloat) -> AttributesItem {
